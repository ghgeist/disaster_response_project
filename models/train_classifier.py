...
def detect_and_replace_urls(text_array):
    ...
    if not isinstance(text_array, np.ndarray):
        raise ValueError(f"Input must be a numpy array of strings, but got {type(text_array)}")
...
def tokenize_text(text, encoding='utf-8'):
    ...
    if isinstance(text, np.ndarray):
        ...
        raise ValueError(f"Input must be a numpy array of strings, but got {type(text)}")
...
def remove_stop_words(words_array, stopwords_set=STOPWORDS_SET):
    ...
    if not isinstance(words_array, np.ndarray):
        raise ValueError(f"Input must be a numpy array of strings, but got {type(words_array)}")
