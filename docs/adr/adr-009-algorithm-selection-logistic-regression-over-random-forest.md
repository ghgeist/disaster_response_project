...
**Threshold evaluation caveat (2026-09-21):** Historical ~65% critical recall was measured on the same frozen eval examples used to select per-label thresholds and therefore was not an independent post-calibration estimate. Going forward, thresholds are tuned on `experiments/experimental_configs/eval_sets/cal_ids.json` (train-side) and reported on frozen `eval_ids.json` only. A three-way retrain of the vocab15k LR path (`03_create_experimental_model.py`, fit excludes `cal ∪ eval`) yielded **61.5%** critical recall on frozen eval under that contract (calibration diagnostic critical recall remained ~65.1%). Frozen eval is report-only for threshold calibration; it is not a pristine never-touched final test set because prior engineering decisions already inspected its results.
